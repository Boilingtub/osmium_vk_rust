    ...
    event_loop.run(move |event, elwt| {
        match event {
            ...
            Event::WindowEvent { event, .. } => match event {
                ...
                // If Window Resized set app.resized = true, swapchain will be recreated in render
...
impl App {
    ...
    unsafe fn create(window: &Window) -> Result<Self> {
        ...
        create_vertex_buffer(&instance, &device, &mut data)?;//Vertex Buffer
    ...
    // Recreates the swapchain for the vulkan program
    ...
    unsafe fn destroy(&mut self) {
        ...
        self.device.destroy_buffer(self.data.vertex_buffer, None); //destroy vertex Buffer
        self.device.free_memory(self.data.vertex_buffer_memory, None); //Free Vertex Buffer Mem
...
unsafe fn create_pipeline(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    let vert = include_bytes!("../resources/shaders/bin/shader_vert.spv");
    let frag = include_bytes!("../resources/shaders/bin/shader_frag.spv");
...
static VERTICES: [Vertex; 3] = [
    Vertex::new( vec2(0.0, -0.9), vec3(1.0, 1.0, 0.0)),
    Vertex::new( vec2(0.9, 0.9),  vec3(0.0, 1.0, 1.0)),
    Vertex::new( vec2(-0.9, 0.9), vec3(1.0, 0.0, 1.0)),
];
